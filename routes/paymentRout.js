...
router.post("/checkout", verifyToken, async (req, res) => {
  ...
  if (req.body.type === "product") {
    ...
    if (dbproduct.quantity < 1)
      return res.status(404).json({
        success: false,
        message: "Sorry! This products is currently out of stock",
      });
    ...
    if (!cartt) {
      return res
        .status(404)
        .json({ message: "no products found on your cart" });
    }

    cartt.products.forEach((product) => {
      //murgind user cart product with db product info like price n all whic are dynamic
      const productInfo = cartt.productInfo.find(
        (info) => `${info._id}` === `${product.productID}`
      ); //converted to string because when i was checking === it was cheecking the refrence on the memory not value bcz its an Objectid is an refrence ty[e]
  ...
  try {
    ...
    console.log(
      "🛺 ~ file: paymentRout.js:108 ~ router.post ~ response:",
      response
    );
    ...
    console.log("🛺 ~ file: paymentRout.js:137 ~ router.post ~ error:", error);
  }
});
    ...
    try {
      ...
      if (dborder.type === "cart") {
        ...
        await User.updateOne(
          ...
          {
            $addToSet: {
              purchasedProducts: { $each: dborder.products.map((p) => p._id) },
            },
          }
